...
/**
 * Opcja dla wariantów, które nie przeszły opcji V1 (plik JS w tym samym
 * katalogu)
...
const reapData = file =>
	getSpreadsheet(file)
		...
		.map(sheet => sheet.getRange('A3:E').getValues())
		...
		.map(([a, b, c, d, e]) => {
			if (typeof e === 'string') {
				 ...
				 * Zapis dla schematu:
				 * method w kolumnie D
				 * Reszta w E wg schematu: Zapis : Całość : A. Local : 1 min
				 */
				const regEx = /(.+) : (.+) : (.+) : (.+)/;
				const [, io, type, structure, frq] = regEx.exec(e) || [
					'',
					'',
					'',
					'',
...
const applyNewReports = folderUrl => {
	...
	getFolders(folderUrl, 'title = "A.1.1 Zapis : Całość : 1 min"')
		.map(folder => ({
			folder,
			// archiveFolderID: createFolder(folder, '_Ver 3').getId(),
			files: getFiles(
				folder,
				'title = "Zapis : Całość : 1 min : Wyniki : A. Local"'
			),
		}))
